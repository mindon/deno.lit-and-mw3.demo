...
export const xstyles = function(names: any, kfr: any) {
  ...
  let rules = [...globalThis.document.styleSheets].filter( ({ href, ownerNode }) => {
...
export const ripplefx = function(selectors: string, root: any) {
  Array.from((root || globalThis.document).querySelectorAll(selectors)).forEach(btn => globalThis.mdc?.ripple.MDCRipple.attachTo(btn));
}
